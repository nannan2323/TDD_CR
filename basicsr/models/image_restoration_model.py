...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('highlow_opt'):
            HL_type = train_opt['highlow_opt'].pop('type')
            cri_highlow_cls = getattr(loss_module, HL_type)
            self.cri_highlow = cri_highlow_cls(
                **train_opt['highlow_opt']).to(self.device)
        else:
            self.cri_highlow = None
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        if self.cri_highlow:
            ...
            for pred in preds:
                l_highlow += self.cri_highlow(pred, self.gt)
                if l_highlow is not None:
                    # l_total += L_highlow
                    loss_dict['l_highlow'] = l_highlow
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                if sr_img.shape[2] == 6:
                    ...
                    # imwrite(gt_img[:,:,[2,1,0]], save_gt_img_path)
    ...
    def save_best(self, epoch,  current_iter):
        self.save_network(self.net_g, f'best_epoch{epoch}_net_g', current_iter)
